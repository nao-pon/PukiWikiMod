...
// $Id: attach.inc.php,v 1.1 2003/06/28 06:01:53 nao-pon Exp $
...
function plugin_attach_convert()
{
	...
	global $xoopsUser,$anon_writable,$X_admin;
	...
	if ($dir = @opendir(UPLOAD_DIR))
	{
		while($file = readdir($dir))
		{
			...
			$open = "<a href=\"$wiki2xoops_tmp?plugin=attach&amp;openfile=${filename_url}&amp;refer=${refername_url}\" title=\"$lastmod $file_size\">$icon$filename</a>$counter\n";
			
			// nao-pon さらに、管理者のみ削除可に変更
...
function plugin_attach_action()
{
	...
	{
		if (!$X_admin) return array("msg" => $_msg_nodel);
		if($vars["mode"] == "confirm") {
			...
			//if (is_freeze($vars["refer"]) || !is_editable($vars["refer"])) return array("msg" => $_msg_nodel);
			if (!$X_admin) return array("msg" => $_msg_nodel);

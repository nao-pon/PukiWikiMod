<?php
// $Id: map.inc.php,v 1.1 2003/06/28 06:01:55 nao-pon Exp $
/*
...
function showReverse(&$pages,$not) {
	...
	foreach (array_keys($pages) as $page) {
		...
		$body .= ' <li class="site_map">'.$pages[$page]->_link;
		if (count($pages[$page]->_from)) {
			if ($not) { $body .= ' is link from'; }
			$body .= "\n  <ul>\n";
			foreach ($pages[$page]->_from as $from)
				$body .= "   <li class='site_map'>{$pages[$from]->_link}</li>\n";
			$body .= '  </ul>';
		}
		$body .= "</li>\n";
	}
...
function showWikiNodes(&$pages,$page,$level) {
	...
	foreach ($pages[$page]->_links['WikiName'] as $_obj) {
		if ($_obj->_exist)
			...
			$body .= " <li class='site_map'>{$_obj->_link}</li>";
	}
	return $body;
}
//HyperLinkを出力
...
function showInterWikiName(&$obj) {
	...
	foreach ($obj->_links['InterWikiName'] as $_obj)
		$body .= " <li class='site_map'>inter{$_obj->_link}</li>\n";
